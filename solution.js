...
allSeatCont.forEach((seat) => {
  ...
  seat.setAttributeNode(attr);    // explain ----------
});
...
seatContEl.forEach((seat) => {
  seat.addEventListener("click",(e) => {
    ...
    let seatId = JSON.parse(seat.dataset.seatid);;/// explain kro

      if (!isSelected) {
        seat.classList.add("selected");
        takenSeats.push(seatId);
        takenSeats = [...new Set(takenSeats)]; // exapian kro
      } else if (isSelected) {
